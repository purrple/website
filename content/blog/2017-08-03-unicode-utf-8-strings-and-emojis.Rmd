...
Typically when I want to understand something, I make an R 📦, I guess I really want 
to understand this, as I am making not 1, not 2 but 3 📦📦📦 (if I count `emojitsu`). 

 - 📦 `uni` : contains a tibble of unicode runes
 - 📦 `utf8splain` : to get to the 0️⃣ and 1️⃣ of utf-8 string encoding
...
tidyverse 🏋, perhaps I'll ✍️ another post about that, but let's not digress more yet. 
...
It needs `r nchar(world_binary_signif)` bits, in terms of utf-8 it means it needs 4 bytes (in red). These 4 bytes contain the utf-8 machinery (the light gray bits) and the actual binary bits for the rune (in black). To go full ⭕️ the `runes` function extract the description of the runes using the 
`uni::code` data 🖼.  
