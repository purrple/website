...
Typically when I want to understand something, I make an R 📦. For example a few years ago I wanted to understand C++. I guess I really want to understand this, as I am making not 1, not 2 but 3 pa📦if I count `emojitsu`). 
...
... and if you use a [crayon](htt 🖍ps://github.com/r-lib/crayon) compatible console, like
...
It needs `r nchar(world_binary_signif)` bits, in terms of utf-8 it means it needs 4 bytes (in red). These 4
 bytes contain the utf-8 machinery (the light gray bits) and the actual binary bits for the rune (in black).
To go full circle ⭕️unes` function extract the description of the runeach runetwith a oleft_join` with the `de` tibble .
